...
def generate_cctv_scan_report(case_id, cctv_id, scan_data, output_dir="reports"):
    ...
    try:
        ...
        cctv_table.setStyle(TableStyle([
        ...
        if scan_data.get('success'):
            ...
            result_style = ParagraphStyle(
                ...
                fontSize=12,
                textColor=colors.HexColor('#2e7d32') if matches_found > 0 else colors.HexColor('#c62828')
            )
            
            if matches_found > 0:
                story.append(Paragraph(f"<b>✓ MATCHES FOUND: {matches_found}</b>", result_style))
            else:
            ...
            if matches_found > 0:
                ...
                match_table = Table(match_data, colWidths=[1.5*inch, 1.5*inch, 1.5*inch, 1.5*inch])
                ...
                for idx, match in enumerate(matches[:3]):  # Show first 3 images
                    ...
                    if img_path and os.path.exists(img_path):
                        try:
                            ...
                            story.append(img)
                            story.append(Paragraph(f"Frame {match['frame']} - Similarity: {match['similarity']:.4f}", styles['Normal']))
                            story.append(Spacer(1, 0.2*inch))
                        except Exception as e:
                            print(f"[WARNING] Could not add image {img_path}: {e}")
        else:
            error_msg = scan_data.get('error', 'Unknown error')
            story.append(Paragraph(f"<b>Scan Failed:</b> {error_msg}", styles['Normal']))
...
def generate_aggregate_report(case_id, scan_task_id, output_dir="reports"):
    ...
    try:
        ...
        scan_task = dict(cursor.fetchone())
        ...
        # Title
        story.append(Paragraph("Missing Person Case - Aggregate Scan Report", title_style))
        story.append(Spacer(1, 0.2*inch))
        ...
        for result in scan_results:
            results_data.append([
                ...
                f"{result['scan_duration_seconds']:.2f}" if result['scan_duration_seconds'] else "N/A",
                "✓ Complete" if result['detections_found'] >= 0 else "✗ Failed"
            ])
        
        results_table = Table(results_data, colWidths=[2*inch, 1*inch, 1*inch, 1*inch, 1.5*inch])
        ...
        if total_detections > 0:
            ...
            conclusion_style = ParagraphStyle('Conclusion', parent=styles['Normal'], fontSize=12, textColor=colors.HexColor('#2e7d32'))
        else:
            conclusion_text = f"No matches found across {scan_task['scanned_cctvs']} CCTV location(s). Monitoring will continue."
            conclusion_style = ParagraphStyle('Conclusion', parent=styles['Normal'], fontSize=12, textColor=colors.HexColor('#d32f2f'))
