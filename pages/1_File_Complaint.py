...
with st.form("complaint_form"):
    ...
    with col1:
        ...
        email = st.text_input(
            "Email for Notifications", placeholder="your-email@example.com"
        )
        ...
        date_part = st.date_input(
            "Date Person Went Missing *", value=default_time.date()
        )
        time_part = st.time_input(
            "Time Person Went Missing *", value=default_time.time()
        )
        time_lost = datetime.datetime.combine(date_part, time_part)

        # Location input options
        location_method = st.radio(
            "Location Input Method", ["Select from List", "Enter Coordinates"]
        )

        if location_method == "Select from List":
            location_names = [loc["name"] for loc in CCTV_LOCATIONS]
            selected_location = st.selectbox(
                "Last Seen Location *", [""] + location_names
            )
            last_seen_location = selected_location
        else:
            last_seen_location = st.text_input(
                "Last Seen Location (Lat, Lon) *",
        ...
        if not name or not uploaded_photo or not last_seen_location:
            ...
            if location_method == "Select from List" and selected_location:
                ...
                try:
                    ...
                    st.error(
                        "Invalid location format. Use 'Lat, Lon' (e.g. 23.2699, 77.4026)"
                    )
                    st.stop()
            ...
            with st.spinner("Processing complaint with AI Agents..."):
                ...
                if result.get("cancelled", False):
                    ...
                    st.info(
                        f"**Reason:** {result.get('cancellation_reason', 'Unknown')}"
                    )
                    st.markdown(
                        """
                ...
                """,
                    (
                        name,
                        age,
                        nlp.get("full_text", description),
                        geo.get("geohash", ""),
                        location_name,
                        time_lost.strftime("%Y-%m-%d %H:%M:%S") if time_lost else None,
            ...
            with col1:
                ...
                if time_analysis:
                    ...
                    st.write(
                        f"**Time Elapsed:** {time_analysis.get('hours_elapsed', 0):.2f} hours"
                    )
                    st.write(
                        f"**Search Radius:** {time_analysis.get('search_radius_km', 0):.2f} km"
                    )
                    st.write(
                        f"**Max Walking Distance:** {time_analysis.get('max_walking_distance_km', 0):.2f} km"
                    )

            with col2:
                st.markdown("### Route Prediction")
                st.write(
                    f"**CCTV Locations to Monitor:** {route.get('num_cctv_locations', 0)}"
                )
                if cctv_videos:
                    st.write("**Cameras:**")
            ...
            if email:
                with st.spinner("Sending email notification..."):
                    ...
                    if notification_result.get("email_sent"):
                        ...
                        st.warning(
                            "⚠️ Email notification failed (check email configuration)"
                        )
            ...
            if cctv_videos and len(cctv_videos) > 0:
                ...
                scan_task_id = start_background_scan(case_id, cctv_videos, photo_path)

                if scan_task_id:
                    st.success(
                        f"✅ Background scan started! Scan Task ID: {scan_task_id}"
                    )
                    st.info(
                        "You will receive an email notification when the scan is complete."
                    )
